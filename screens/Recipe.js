...
const Recipe = ({ route }) => {
  ...
  const getUser = async () => {
    ...
    const document = await fb.firestore().collection("users").doc(user.uid)
      .get();
    if (
      document.data().favorites.find((favorite) =>
        favorite.id === route.params.id
      )
    ) {
      setLiked(true);
      setLoader(true);
  ...
  const getRecipe = async () => {
    ...
    const complexURL =
      `https://api.spoonacular.com/recipes/${idFood}/information?apiKey=fe730f254f2c4dae876e87a2cd27338d`;
        ...
        <>
          <View style={{ height: "50%" }}>
            {loader
              ...
              : <Feather name="loader" style={styles.buttonText2} />}
          </View>
          <ScrollView style={styles.info}>
            ...
            {loader === false
              ...
              : (
                <TouchableOpacity
                  onPress={() => setFavorite(route.params.id, recipe.title)}
